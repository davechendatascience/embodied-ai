...
| 目標開口＋吸附解碼 | 69%（抽屜 13/20） | 5.5% |
| 夾爪分類（等權重） | 46% | — |
| 夾爪分類（CE ×0.1、以 twist 選 checkpoint） | 75.5%（抽屜 16/20） | 4% |
| **＋教師最低接近速度 5 cm/s，DAgger round 3（200 回合）** | **87.5%**（10 個任務有 8 個 ≥ 80%，抽屜 19/20） | **7.5%** |

盲控制組維持在接近 0，代表成功來自視覺，而非記住軌跡。
剩下的主要失敗是任務 4（從打開的抽屜裡拿碗）：round 2 只有 2/20。

**夾爪介面的演進**：三值指令 → 目標開口（GripperServo）→ 對程式使用的開口做分類（0、26、80 mm），夾爪損失權重 0.1、以 twist 損失挑 checkpoint。

**教師最低接近速度**：教師原本以比例控制收斂，最後 1–3 cm/s，學生在該速度下的方向與教師只有 cosine 0.68，閉環會在距離幾 mm 處停住或龜速爬行。加入 5 cm/s 下限後教師仍 47/50 成功，學生從 75.5% 提升到 87.5%。
...
因此改成與手臂相同的做法：策略輸出**目標開口（公尺）**，由固定的 `GripperServo` 讀取夾爪實測開口與速度產生指令（`screwhead/sim/gripper_servo.py`）。教師經過 servo 執行仍 8/8 成功。
...
- **第 2 關（視覺落地）已完成**：學生 0.87、盲控制組失敗 0.92、指令關係 1.00 皆 supported；教師逐任務 10 個中 7 個 supported（任務 1 為 0.71，低於 0.75 門檻）。
- **第 2b 關（泛化）已量測**：訓練 7 個任務、評估 3 個沒看過的任務。抽出 0/3/7 得 53%（supported）、抽出 2/5/9 得 32%（contested）。桌面上的碗 11–17/20、低支撐物上的碗 8–15/20，但兩個爐台任務都是 0/20（教師 0.87–0.92、全任務學生 15–18/20）。
- **結論**：不能泛化的是「對沒看過的支撐高度的接近方式」，而不是取放行為本身 → 下一步是以幾何參數化的技能庫（`CMP-skill-library`），而不是再多收資料。
