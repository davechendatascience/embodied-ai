...
def _record(item: tuple) -> str:
    suite, task, episode, seed, px, out, cpu = item
    ...
    try:
        ...
        for _ in range(episode):                   # the stream's earlier episodes, unsimulated
            env.skip_episode()
    ...
    path = out / suite / f"{suite}_t{task}_ep{episode}_{tag}.mp4"
...
def main() -> int:
    ...
    items = [(s, t, args.episode, args.seed, args.px, out)
