...
def write(language: str, placements: list[Placement], goal: list[tuple], interest: list[str] | None = None) -> str:
    ...
    from libero.libero.utils.bddl_generation_utils import (get_affordance_region_kwargs_list_from_fixture_info, get_result,
                                                          ...
                                                          kitchen_table_task_suites_generator)
    ...
    text = kitchen_table_task_suites_generator(
        language=language,
        xy_region_kwargs_list=get_xy_region_kwargs_list_from_regions_info(regions),
        # each object's own regions (the basket's contain_region), declared as LIBERO's scene templates declare them
        affordance_region_kwargs_list=get_affordance_region_kwargs_list_from_fixture_info(
            {p.name: affordances[p.category] for p in placements if p.category in affordances}),
        fixture_object_dict={WORKSPACE: [WORKSPACE]},
        movable_object_dict=movable,
        objects_of_interest=list(interest if interest is not None else [p.name for p in placements]),
        init_states=[("On", p.name, f"{WORKSPACE}_{p.region}") for p in placements],
